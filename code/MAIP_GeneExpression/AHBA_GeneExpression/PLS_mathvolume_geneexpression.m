...
% save as txt for further analysis
save(fullfile(output_path,'CCA_math_brainmap_N219.txt'),'math_related_brainmap','-ascii','-double');
...
X=gene_regional_expression(num_region,:); % Predictors
...
output_name = fullfile(output_path,'PLS1_brainloading.nii');
...
output_figure = fullfile(output_path,'PLS1_brainloading.jpg');
BrainNet_MapCfg('BrainMesh_ICBM152_smoothed.nv',output_name,cfg_file,output_figure);
save(fullfile(output_path,'PLS1_brainloading.txt'),'brainmeasure1','-ascii','-double');
...
%% visualization
load(output_name,'culmulate_p','R','perm_Rsq_all');

color = [217,237,232];
output_name = fullfile(output_path,'permutation_Rsquare.tiff');
histogram_Jin(perm_Rsq_all(:,3), color, 'Variance explained', 'Frequency', 'Permutation',output_name)
a=xline(R(3));
a.LineWidth = 3;
a.Color = 'r';
